...
{ 
    ...
    */
    ...
    while (done == 0) {
        ...
        for (int i = 0; i < n; i++) {
            // If this process still has burst time remaining
            if (rem_bt[i] > 0) {
...
// Function to find the waiting time for all  
// processes using Shortest Job First (Preemptive - SRTF)
...
{ 
    ...
    */
    ...
    int t = 0;  // Current time
    int minm = INT_MAX;  // Minimum remaining time
    int shortest = 0;  // Index of process with shortest remaining time
    int finish_time;
    int check = 0;  // Flag to check if a process is found
    
    // Process until all processes are completed
    while (complete != n) {
        // Find process with minimum remaining time among processes that have arrived
        minm = INT_MAX;
        check = 0;
...
{ 
    ...
    // So if p1 has lower priority value, it should come after p2
    if (p1->pri < p2->pri)
        return 1;
